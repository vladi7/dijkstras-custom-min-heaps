...
import fibheap as fb #insert-O(1) Extract-min O(logn)
#binomial insert(logn), remove(logn) https://stackoverflow.com/questions/6215485/what-is-the-difference-between-binary-heaps-and-binomial-heaps
class Vertex:
...
if __name__ == '__main__':
    ...
    # adding vertices(zip codes) and edges(distances between the zip codes)
    g = Graph()
    ...
    for request in requests:
        ...
        #set all distances in graph to infinity
        if len(freeVehicles) > 0:
            g.resetVertices() 
            vertex = g.getVertex(request['zip']) 
            dijkstra(g, vertex) 
            #assign calculated distances to each availible vehicle
            for av in freeVehicles:
                av['dist'] = g.getVertex(av['zip']).getdist()
            #sort free vehicles by the distance
